...
{
    public class NFeSignatureRepositorio : IDisposable, INFeSignatureRepositorio
    {
        private EFContext _context;

        #region .: Construtor :.

        public NFeSignatureRepositorio(EFContext context)
        {
            _context = context;
        }
        ...
        public List<NFeSignature> GetAllItems()
        {
            return _context.NFeSignature.ToList();
        }

        public NFeSignature Add(NFeSignature valor)
        {
            _context.NFeSignature.Add(valor);
            _context.SaveChanges();

            return valor;
        }

        public NFeSignature GetById(int id)
        {
            return _context.NFeSignature.Where(x => x.Id.Equals(id)).FirstOrDefault();
        }
