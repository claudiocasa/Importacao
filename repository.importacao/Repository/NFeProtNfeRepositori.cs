...
{
    public class NFeProtNfeRepositorio : IDisposable, INFeProtNfeRepositorio
    {
        private EFContext _context;

        #region .: Construtor :.

        public NFeProtNfeRepositorio(EFContext context)
        {
            _context = context;
        }
        ...
        public List<NFeProtNfe> GetAllItems()
        {
            return _context.NFeProtNfe.ToList();
        }

        public NFeProtNfe Add(NFeProtNfe valor)
        {
            _context.NFeProtNfe.Add(valor);
            _context.SaveChanges();

            return valor;
        }

        public NFeProtNfe GetById(int id)
        {
            return _context.NFeProtNfe.Where(x => x.Id.Equals(id)).FirstOrDefault();
        }
